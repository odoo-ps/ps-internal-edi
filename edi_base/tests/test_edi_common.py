...
class TestEDICommon(TransactionCase):
    """
    Common class to be inherited by EDI tests for in and out flows.
    ...
    synchronizations created by individual tests are correctly cleaned.
    ...
    `edi.synchronization` records generated by the integration, we need to create
    ...
    def _clean_synchronizations(self):
        ...
        with registry(self.env.cr.dbname).cursor() as cr:
            ...
            imds = env["ir.model.data"].search([("model", "=", "edi.integration")])
            integrations = (
                env["edi.integration"].with_context(active_test=False).search([("id", "not in", imds.mapped("res_id"))])
            )
            synchronizations = (
                env["edi.synchronization"]
                .with_context(active_test=False)
                .search([("integration_id", "in", integrations.ids)])
            )
