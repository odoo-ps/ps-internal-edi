...
class Integration(models.Model):
    ...
    reasonable_execution_time = fields.Float(default=10, help="""
...
class Synchronization(models.Model):
    ...
    def _compute_execution_time(self):
        for rec in self:
            rec.execution_time = (rec.synchronization_end_date - rec.synchronization_date).total_seconds() / 60 \
                if rec.synchronization_date and rec.synchronization_end_date else False
